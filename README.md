...
TRANSFER_SPREADSHEET_ID=234567890123456789234567890123456789
...
# /transfer の実行を許可するチャンネルID（カンマ区切り）
...
* `TRANSFER_ALLOWED_CHANNEL_IDS` を設定すると、そのチャンネルでのみ `/transfer` を実行できます
* `sheets` はテンプレートを除くシート名だけを一覧表示します
* `add` の追加先行は `B:D` の実データだけで末尾判定するため、A列のプルダウン設定は影響しません
* `add` は E列の備考も任意で書き込めます
* `check` のユーザー一覧は A列が `特別枠` / `普通枠` の行だけを対象にし、F列の招待済み状態も表示します
* `invited` は一致したユーザーID行の F列に `◯` を記入します
* `uninvited` は A列が `特別枠` / `普通枠` かつ F列が空欄の行だけを表示します
